...
def gerar_png_recibo_medico():
    ...
    y += 10
    y = linha(
        draw, y, "Dra. Fernanda Costa Oliveira  -  CRM/AM 18.765", size=17, bold=True
    )
    y = linha(
        draw, y, "Av. Djalma Batista, 1010, Sala 305 - Chapada - Manaus/AM", size=14
    )
    ...
    y = linha(
        draw,
        y,
        "IRPF: Despesas médicas são dedutíveis - código 21 (Médicos).",
        size=14,
        cor=(0, 120, 0),
    )
    ...
    y += 30
    draw.text(
        (50, y), "Manaus, 14 de março de 2024.", fill=(60, 60, 60), font=fonte(14)
    )
    y += 50
    ...
    draw.text(
        (50, y),
        "Dra. Fernanda Costa Oliveira  -  CRM/AM 18.765",
        fill=(60, 60, 60),
        font=fonte(13),
    )
    y += 40
...
def gerar_png_plano_saude():
    ...
    y += 10
    y = linha(
        draw, y, "UNIMED MANAUS COOPERATIVA DE TRABALHO MÉDICO", size=17, bold=True
    )
    ...
    y = linha(
        draw,
        y,
        "IRPF: Planos de saúde são dedutíveis - código 26 (Planos de Saúde).",
        size=14,
        cor=(0, 120, 0),
    )
    y = linha(
        draw,
        y,
        "Inclua também os valores pagos pelos dependentes.",
        size=14,
        cor=(0, 120, 0),
    )
    y += 30
...
def gerar_pdf_mensalidade_pedro():
    ...
    pdf.cell(0, 6, "Rua da Educação, 800 - Aleixo - Manaus/AM", ln=True, align="C")
    pdf.ln(4)
    ...
    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(60, 7, "Responsável:", ln=False)
    pdf.set_font("Helvetica", "", 11)
    ...
    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(60, 7, "Série/Turma:", ln=False)
    pdf.set_font("Helvetica", "", 11)
    pdf.cell(0, 7, "6 Ano do Ensino Fundamental - Turma C", ln=True)

    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(60, 7, "Competência:", ln=False)
    pdf.set_font("Helvetica", "", 11)
    ...
    pdf.set_font("Helvetica", "", 11)
    pdf.cell(0, 7, "Boleto bancário - 05/10/2024", ln=True)
    ...
    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(
        0, 7, "Código do Serviço: 8.01 - Ensino regular reconhecido pelo MEC", ln=True
    )
    pdf.ln(2)
    ...
    pdf.multi_cell(
        0,
        6,
        "IRPF: Despesas de educação são dedutíveis - código 01 (Instrução).\n"
        "Limite anual: R$ 3.561,50 por pessoa. Lançamento em nome do dependente.",
    )
    pdf.set_text_color(0, 0, 0)
    pdf.ln(6)
    ...
    pdf.ln(4)
    pdf.cell(0, 6, "Setor Financeiro - Colégio Estadual Progresso", ln=True)
    pdf.ln(8)
    ...
    pdf.cell(
        0,
        6,
        "DOCUMENTO FICTÍCIO PARA FINS DE TESTE DE SISTEMA - NÃO TEM VALIDADE FISCAL",
        ln=True,
    )
...
def gerar_pdf_conta_hospitalar():
    ...
    pdf.set_font("Helvetica", "B", 16)
    pdf.set_text_color(20, 80, 160)
    pdf.cell(0, 10, "CONTA HOSPITALAR - INTERNAÇÃO", ln=True, align="C")
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Helvetica", "", 11)
    pdf.cell(0, 6, "HOSPITAL E CLÍNICA SÃO LUCAS LTDA", ln=True, align="C")
    pdf.cell(0, 6, "CNPJ: 34.678.901/0001-55", ln=True, align="C")
    pdf.cell(
        0, 6, "Av. Carvalho Leal, 1456 - Cachoeirinha - Manaus/AM", ln=True, align="C"
    )
    pdf.ln(4)
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(6)
    ...
    campos = [
        ("Paciente:", "ANA CLARA RODRIGUES NASCIMENTO"),
        ("CPF:", "071.234.567-18"),
        ("Prontuário:", "HC-2024-008821"),
        ("Internação:", "18/11/2024  a  22/11/2024  (4 diárias)"),
        ("Diagnóstico:", "Apendicite aguda - CID K35.8"),
        ("Cirurgia:", "Apendicectomia laparoscópica"),
    ]
    for label, valor in campos:
        pdf.set_font("Helvetica", "B", 11)
    ...
    pdf.cell(0, 8, "DISCRIMINAÇÃO DOS VALORES", ln=True)
    pdf.ln(2)

    itens = [
        ("Diárias hospitalares (4x R$ 1.200,00)", "R$  4.800,00"),
        ("Honorários médicos - cirurgia", "R$  3.500,00"),
    ...
    pdf.cell(
        0,
        6,
        "Pago pelo plano de saúde: R$ 9.552,00  |  Coparticipação: R$ 2.388,00",
        ln=True,
    )
    pdf.ln(4)
    ...
    pdf.line(10, pdf.get_y(), 100, pdf.get_y())
    pdf.ln(4)
    pdf.cell(0, 6, "Faturamento - Hospital e Clínica São Lucas", ln=True)
    pdf.ln(8)
    pdf.set_text_color(180, 0, 0)
    pdf.set_font("Helvetica", "B", 9)
    pdf.cell(
