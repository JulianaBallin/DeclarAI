...
\hypersetup{
  ...
  pdftitle={Relatório Técnico DeclarAI},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textbf{\color{declaraTeal}DeclarAI}}
...
\begin{titlepage}
  \centering
  \vspace*{18mm}
  {\Huge\bfseries\color{declaraTeal}DeclarAI\par}
  \vspace{4mm}
...
\section{Matriz de Atendimento aos Requisitos}

\begin{longtable}{@{}L{44mm}L{24mm}L{88mm}@{}}
\toprule
\textbf{Requisito da atividade} & \textbf{Status} & \textbf{Evidência no projeto} \\
\midrule
\endhead
...
Base de conhecimento própria & Atendido & A base contém \texttt{guia\_imposto\_renda.txt} e \texttt{pr-irpf-2024.pdf}, ambos justificados por relevância fiscal. \\
...
Embeddings e recuperação & Atendido & Usa embeddings \texttt{MiniLM} multilíngues, ChromaDB, busca vetorial e re-ranking com CrossEncoder. \\
...
\subsection{Justificativa para Execução Local}
...
\subsubsection*{Modelo de Geração e Classificação (LLM)}
...
\toprule
\textbf{Modelo} & \textbf{Cobertura (\%)} & \textbf{Latência (s)} & \textbf{Observação} \\
\midrule
...
\subsubsection*{Modelo de Embeddings}

O \texttt{sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2} foi escolhido por três razões: (1) suporte nativo ao português brasileiro, sem necessidade de tradução prévia; (2) dimensionalidade de 384, adequada para ChromaDB em CPU; e (3) tamanho reduzido, permitindo execução eficiente em máquinas sem GPU.

\subsubsection*{Modelo de Re-ranking}
...
\begin{itemize}[leftmargin=7mm]
  ...
  \item Upload de documentos com perfil do declarante, verificação de titularidade e revisão antes de salvar;
  \item Base de conhecimento para adicionar, remover e re-indexar fontes;
  \item Histórico de documentos salvos, com resumo anual e economia estimada;
  \item Avaliação do pipeline, com modo de recuperação e modo completo com LLM;
  \item Status do sistema, incluindo Ollama, modelos e chunks indexados.
\end{itemize}

\section{Diagramas e Identidade Visual}
...
\begin{center}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash\ttfamily\scriptsize}p{92mm}Y@{}}
\toprule
\textbf{Arquivo} & \textbf{Conteúdo} \\
\midrule
docs/diagrams/logo.svg & Logo horizontal do DeclarAI. \\
docs/diagrams/logo\_icon.svg & Ícone usado no frontend. \\
docs/diagrams/rag/recuperacao\_reranking.svg & Fluxo de recuperação, re-ranking e geração da resposta. \\
docs/diagrams/agentic\_rag/titularidade\_justificativa.svg & Fluxo agentic de upload, titularidade, justificativa e salvamento. \\
docs/diagrams/rag/indexacao\_e\_ingestao.svg & Ingestão, limpeza, chunking, embeddings e ChromaDB. \\
docs/diagrams/agentic\_rag/ferramentas\_agente.svg & Ferramentas disponíveis ao agente. \\
\bottomrule
\end{tabularx}
\end{center}
...
O dataset \texttt{data/eval/perguntas.json} possui 60 perguntas anotadas com resposta de referência, palavras-chave e nível de dificuldade distribuídos em 12 categorias fiscais: obrigatoriedade, deduções médicas, educação, previdência privada, rendimentos, prazos, penalidades, autônomos, dependentes, aluguéis, doações e documentos fiscais.
...
\subsection{Comparação de Estratégias de Chunking}
...
\begin{center}
\begin{tabularx}{\textwidth}{@{}L{36mm}L{22mm}L{22mm}Y@{}}
\toprule
\textbf{Configuração} & \textbf{Tamanho} & \textbf{Sobreposição} & \textbf{Observação} \\
\midrule
fixo\_200\_0   & 200 & 0   & Trechos curtos; perde contexto de regras fiscais \\
fixo\_400\_40  & 400 & 40  & Equilíbrio fraco; sobreposição insuficiente \\
\textbf{fixo\_600\_80}  & \textbf{600} & \textbf{80}  & \textbf{Melhor equilíbrio: contexto fiscal + precisão semântica} \\
fixo\_800\_120 & 800 & 120 & Contexto rico, mas maior ruído semântico nos embeddings \\
fixo\_1000\_200 & 1000 & 200 & Trechos longos demais; diluem a similaridade vetorial \\
...
\begin{center}
\begin{tabularx}{\textwidth}{@{}L{54mm}L{32mm}Y@{}}
\toprule
\textbf{Validação} & \textbf{Resultado} & \textbf{Observação} \\
\midrule
\texttt{pytest -v} & Aprovado & 6 testes passaram e 1 teste de extração completa foi pulado na \texttt{venv} leve por ausência de \texttt{pdfplumber}. \\
\texttt{make check} & Aprovado & \texttt{isort}, \texttt{black} e \texttt{flake8} sem pendências no backend. \\
\texttt{npm run build} & Aprovado & Build Vite gerado sem erro. \\
\texttt{npm run lint} & Aprovado & ESLint sem avisos ou erros. \\
Docker Compose & Aprovado & Backend e frontend saudáveis, Ollama disponível e modelo \texttt{mistral} instalado. \\
Fluxo de upload & Aprovado & Recibo odontológico de dependente processado com beneficiário correto, categoria \texttt{Recibo Médico} e justificativa enriquecida. \\
...
Avaliação de recuperação & Aprovado & Execução pela API com 60 casos retornou 100\% de recuperação, score médio de 0,6253 e nenhum caso sem contexto. \\
\bottomrule
\end{tabularx}
\end{center}
